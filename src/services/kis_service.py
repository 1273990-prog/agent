...
class KisService(BaseService):
    ...
    def access_token_to_json(self, model_json: str) -> Dict[str, Any]:
        # Implementation remains unchanged...
        pass

    def get_access_token_to_json(self, model_json: str) -> Dict[str, Any]:
        try:
            token_string = self.__get_access_token(model_json)
    ...
    def get_kospi_index_to_json(self, model_json: str) -> Dict[str, Any]:
        try:
            token_string = self.__get_kospi_index(model_json)
            return json.loads(token_string)
        except Exception as e:
            print(e)
            raise e
    ...
    def get_stock_price_to_json(self, model_json: str) -> Dict[str, Any]:
        try:
            token_string = self.__get_stock_price(model_json)
            return json.loads(token_string)
        except Exception as e:
            print(e)
            raise e
    ...
    def __get_access_token(self, model_json: str) -> str:
        try:
            ...
            model_data["service_map"]["appsecret"] = model_data.get("secretkey", "")

            # 부모 클래스(BaseAgent)의 규격에 맞게 다시 JSON 문자열로 직렬화하여 바인딩
            data_map_json = json.dumps(model_data["service_map"])

            print(f"target_data_json\n{data_map_json}")

            # 부모의 동기식 HTTP 요청 처리 메서드 작동
            net_value_json = json.dumps(net_value)
            response_string = self._run(net_value_json, data_map_json)
