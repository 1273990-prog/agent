...
class KrxService(BaseService):
    def __init__(self):
        super().__init__()
        self.__url_base: str = "https://data-dbg.krx.co.kr/svc"

    def get_drvprod_index_to_json(self, model_json: str) -> Dict[str, Any]:
        try:
            return self._execute_and_convert(self.__get_drvprod_index, model_json, to_json=True)
        except Exception as e:
            print(e)
            raise e

    def get_drvprod_index_to_string(self, model_json: str) -> str:
        try:
            return self._execute_and_convert(self.__get_drvprod_index, model_json, to_json=False)
        except Exception as e:
            print(e)
            raise e

    def __get_drvprod_index(self, model_json: str) -> str:
        try:
